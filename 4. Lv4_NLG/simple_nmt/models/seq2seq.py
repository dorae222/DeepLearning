...
class Encoder(nn.Module):
    ...
    def forward(self, emb):
        ...
        y, h = self.rnn(x)
        # h = last time-step's hidden state and cell state
        # |y| = (batch_size, length, (hidden_size/2)*2)
...
class Decoder(nn.Module):
    ...
    def forward(self, emb_t, h_t_1_tilde, h_t_1):
        # |emb_t| = (batch_size, 1, word_vec_size) one time-step
        # |h_t_1_tilde| = (batch_size, 1, hidden_size) # h_t_1_tilde = h_t-1 tilde
        ...
        if h_t_1_tilde is None:
            ...
            h_t_1_tilde = emb_t.new(batch_size, 1, hidden_size).zero_() # make same device & type
...
class Generator(nn.Module):
    ...
    def __init__(self, hidden_size, output_size):
        ...
        self.softmax = nn.LogSoftmax(dim=-1) # dim=-1 -> output_size
